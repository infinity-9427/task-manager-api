...
export const initializeSocketHandlers = (io: Server) => {
  ...
  io.on('connection', (socket: AuthenticatedSocket) => {
    ...
    socket.on('update_presence', async (status: 'online' | 'away' | 'busy') => {
      try {
        await prisma.user.update({
...
export const emitNotificationToUser = (io: Server, userId: number, notification: any) => {
  io.to(`user:${userId}`).emit('notification', notification);
};

// Helper function to emit task updates to all watchers
export const emitTaskUpdate = (io: Server, taskId: number, update: any) => {
